...
class SocialAccount extends Model
{
    ...
    {
        return $this->belongsToMany(User::class, 'social_account_accesses')
            ->withPivot([
                'granted_by_user_id', 'can_view_omnichat', 'can_reply_omnichat',
                'can_assign_conversations', 'can_access_content', 'can_create_posts', 'can_edit_posts',
                'can_approve_posts', 'can_publish_posts', 'can_delete_posts',
            ])
            ->withTimestamps();
    }
    ...
    public function groups(): BelongsToMany
    {
        return $this->belongsToMany(
            SocialAccountGroup::class,
            'social_account_group_members',
        );
    }

    public function omnichatConversations(): HasMany
    {
        return $this->hasMany(OmnichatConversation::class);
    }

    public function omnichatChannels(): HasMany
    {
    ...
    {
        if ($user->currentWorkspace && $user->can('manageAccounts', $user->currentWorkspace)) {
            return $query;
        }

        return $query->where(function (Builder $query) use ($user): void {
            $query->where('connected_by_user_id', $user->id)
                ->orWhereHas('sharedUsers', fn (Builder $query): Builder => $query->whereKey($user->id));
        });
    }

    public function scopeOmnichatAccessibleBy(Builder $query, User $user): Builder
    {
        $query->where('platform', '!=', SocialPlatform::WordPress->value);
        ...
        return $query->where(function (Builder $query) use ($user): void {
            $query->where('connected_by_user_id', $user->id)
                ->orWhereHas('sharedUsers', fn (Builder $query): Builder => $query
                    ->whereKey($user->id)
                    ->where('social_account_accesses.can_view_omnichat', true));
        });
    }

    public function userHasAccess(User $user, string $permission): bool
    {
        if ($this->connected_by_user_id === $user->id) {
            return true;
        }
