...
use App\Services\Social\WordPressPublisher;
...
class PublishToSocialPlatform implements ShouldBeUnique, ShouldQueue
{
    ...
    private function getPublisher(): mixed
    {
        return match ($this->postPlatform->platform) {
            ...
            SocialPlatform::WordPress => app(WordPressPublisher::class),
        };
    }
    ...
    {
        $post->loadMissing('user', 'workspace.owner');
        $recipients = collect([$post->user, $post->workspace->owner])
            ->filter()
            ->unique('id');
    ...
    private function notifyFailure(Post $post): void
    {
        ...
        foreach ($recipients as $recipient) {
            SendNotification::dispatch(
                ...
                body: "Thất bại trên: {$failedPlatforms}",
                data: ['post_id' => $post->id],
