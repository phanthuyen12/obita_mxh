...
enum Platform: string
{
     ...
     * Facebook-linked) share a key for grouping related account types in the UI.
     */
    ...
    /**
     * All platform values that share this platform's network.
     *
    ...
    public function label(): string
    {
        return match ($this) {
            ...
            self::Lazada => 'Lazada',
            self::Shopee => 'Shopee',
            self::WordPress => 'Website WordPress',
        };
    }
    ...
    public function allowedMediaTypes(): array
    {
        return match ($this) {
            ...
            self::ZaloOa => [MediaType::Image],
            self::Lazada => [MediaType::Image],
            self::Shopee => [],
            self::WordPress => [MediaType::Image, MediaType::Video],
        };
    }
    ...
    public function altTextMaxLength(): ?int
    {
        return match ($this) {
            ...
            self::Discord => 1024,
            self::WordPress => 1000,
            self::TikTok, self::YouTube, self::Telegram, self::ZaloOa, self::Lazada, self::Shopee => null,
        };
    }
    ...
    public function requiredPublishScopes(): array
    {
        return match ($this) {
            ...
            self::Telegram => [],
            self::Discord => [],
            self::ZaloOa => ['oa.user', 'oa.message'],
            self::Lazada => ['im'],
            self::Shopee => ['sellerchat'],
            self::WordPress => [],
        };
    }
    ...
    {
        return ! in_array($this, [self::ZaloOa, self::Lazada, self::Shopee], true);
    }

    /**  list<string> */
    public static function publishingValues(): array
    {
        return array_values(array_map(
            fn (self $platform): string => $platform->value,
            array_filter(self::cases(), fn (self $platform): bool => $platform->supportsPublishing()),
        ));
    }
