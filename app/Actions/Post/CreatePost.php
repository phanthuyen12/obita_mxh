...
class CreatePost
{
     ...
     *     folder_id?: ?string,
     *     content_clone_campaign_id?: ?string,
    ...
    public static function execute(Workspace $workspace, User $user, array $data): Post
    {
        ...
        $post = DB::transaction(function () use ($workspace, $user, $data, $scheduledAt): Post {
            $post = $workspace->posts()->create([
                'user_id' => $user->id,
                'content_workflow_id' => data_get($data, 'content_workflow_id'),
                'folder_id' => data_get($data, 'folder_id'),
                'content_clone_campaign_id' => data_get($data, 'content_clone_campaign_id'),
                'content' => data_get($data, 'content', ''),
                'media' => data_get($data, 'media', []),
                'status' => PostStatus::Draft,
                'is_ceo_content' => (bool) data_get($data, 'is_ceo_content', false),
                'created_via' => data_get($data, 'created_via'),
                'scheduled_at' => $scheduledAt,
            ]);

            SyncPostPlatforms::execute($post, $user);
            ...
            SyncTags::execute($workspace, $post, (array) data_get($data, 'topic_tags', []));

            SyncPostMediaTags::execute(
                $workspace,
                (array) data_get($data, 'media', []),
                (array) data_get($data, 'topic_tags', []),
            );

            $post->load('tags');

            self::notifyWorkflowMembers($post, $workspace, data_get($data, 'content_workflow_id'));
    ...
    private static function notifyWorkflowMembers(Post $post, Workspace $workspace, ?string $workflowId): void
    {
        if (! $workflowId) {
            return;
        }

        $workflow = ContentWorkflow::query()
            ->whereKey($workflowId)
        ...
        foreach ($workflow->members as $member) {
            ...
            Notification::create([
                ...
                'title' => 'Nhiệm vụ viết content mới',
                'body' => "Bạn được giao viết content cho một bài trong luồng {$workflow->name}.",
                'data' => [
                    'post_id' => $post->id,
