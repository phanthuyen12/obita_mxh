...
class ConnectionVerifier
{
    ...
    private function callVerifyEndpoint(SocialAccount $account): bool
    {
        return match ($account->platform) {
            ...
            Platform::ZaloOa, Platform::Lazada, Platform::Shopee => $this->verifyStoredToken($account),
        };
    }

    /**
     * Chat-only providers use the stored token expiry as their connection health signal.
     *
     * @throws TokenExpiredException
     */
    private function verifyStoredToken(SocialAccount $account): bool
    {
        if ($account->is_token_expired) {
            throw new TokenExpiredException("{$account->platform->label()} access token expired");
        }
