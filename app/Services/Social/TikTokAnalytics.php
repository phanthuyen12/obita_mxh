    ...
    public function fetchPostMetrics(PostPlatform $postPlatform): array
    {
        ...
        $response = $this->getHttpClient()
            ->post("{$this->baseUrl}/video/query/?fields=id,like_count,comment_count,share_count,view_count", [
                'filters' => [
                    'video_ids' => [$postPlatform->platform_post_id],
                ],
            ]);
        ...
        if (! is_array($video)) {
            return ['unsupported' => true, 'reason' => 'post_not_found'];
        }

        return [
            ['label' => __('analytics.metrics.views'), 'value' => (int) data_get($video, 'view_count', 0)],
            ['label' => __('analytics.metrics.likes'), 'value' => (int) data_get($video, 'like_count', 0)],
            ['label' => __('analytics.metrics.comments'), 'value' => (int) data_get($video, 'comment_count', 0)],
            ['label' => __('analytics.metrics.shares'), 'value' => (int) data_get($video, 'share_count', 0)],
        ];
    }
