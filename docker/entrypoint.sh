...
# 6) Wait for MySQL to be reachable.
DB_HOST_VALUE="${DB_HOST:-mysql}"
DB_PORT_VALUE="${DB_PORT:-3306}"
DB_USER_VALUE="${DB_USERNAME:-trypost}"
...
WAIT_ATTEMPTS=0
until mysqladmin ping -h "${DB_HOST_VALUE}" -P "${DB_PORT_VALUE}" -u"${DB_USER_VALUE}" -p"${DB_PASSWORD_VALUE}" --silent >/dev/null 2>&1; do
    WAIT_ATTEMPTS=$((WAIT_ATTEMPTS + 1))
    if [ "${WAIT_ATTEMPTS}" -gt 60 ]; then
        echo "[entrypoint] mysql not reachable after 60s — continuing anyway"
        break
    fi
    sleep 1
done
