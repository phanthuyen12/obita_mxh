...
import { update as updateAssetTags } from '@/routes/app/assets/tags';
import {
...
const uploadsFromDate = ref('');
const uploadsToDate = ref('');
const uploadsTag = ref('');
const availableMediaTags = ref<string[]>([]);
...
const fetchUploads = async (page: number, term: string) => {
    ...
    if (uploadsFromDate.value) queryParams.from = uploadsFromDate.value;
    if (uploadsToDate.value) queryParams.to = uploadsToDate.value;
    if (uploadsTag.value.trim()) queryParams.tag = uploadsTag.value.trim();
    if (props.folderId) queryParams.folder_id = props.folderId;

    const response = await fetch(assetsSearch.url({ query: queryParams }), {
        headers: {
...
const loadUploadsFirstPage = async () => {
    ...
    try {
        ...
        availableMediaTags.value = response.tag_options ?? [];
...
watch(
    [uploadsFromDate, uploadsToDate, uploadsTag],
    () => void loadUploadsFirstPage(),
);
watch(
    () => props.folderId,
    ...
    for (const file of files) {
        try {
            await uploadChunked({
                ...
                folderId: props.folderId === 'unfiled' ? null : props.folderId,
            });
        } catch (error) {
...
const tagsTarget = ref<AssetMedia | null>(null);
const selectedAssetTags = ref<string[]>([]);
const openTagsDialog = (asset: AssetMedia) => {
    tagsTarget.value = asset;
    selectedAssetTags.value = [...asset.tags];
    tagsDialogOpen.value = true;
};
const toggleAssetTag = (tag: string) => {
    selectedAssetTags.value = selectedAssetTags.value.includes(tag)
        ? selectedAssetTags.value.filter((value) => value !== tag)
        : [...selectedAssetTags.value, tag];
};
const saveTags = async () => {
    if (!tagsTarget.value) return;
    tagsSaving.value = true;
    const response = await fetch(updateAssetTags.url(tagsTarget.value.id), {
        method: 'PUT',
        credentials: 'same-origin',
        headers: {
            Accept: 'application/json',
        ...
        body: JSON.stringify({ tags: selectedAssetTags.value }),
    });
    tagsSaving.value = false;
    if (!response.ok) {
        toast.error('Không thể lưu thẻ ảnh.');
        return;
    }
    const payload = await response.json();
    const index = uploads.value.findIndex(
        (asset) => asset.id === tagsTarget.value?.id,
    );
    if (index >= 0) uploads.value[index] = payload.data;
    tagsDialogOpen.value = false;
    toast.success('Đã cập nhật thẻ ảnh.');
};
...
const saveMediaFromUrl = async (payload: {
    ...
    httpSaveFromUrl.folder_id =
        props.folderId === 'unfiled' ? null : props.folderId;
    try {
    ...
    <div>
        <Tabs default-value="uploads">
            ...
            <TabsContent value="uploads" class="mt-6">
                ...
                <div
                    class="mb-4 grid gap-3 md:grid-cols-[minmax(220px,1fr)_220px_auto] md:items-center"
                >
                    ...
                    <select
                        v-model="uploadsTag"
                        class="h-12 rounded-md border bg-background px-3 text-sm"
                    >
                        <option value="">Tất cả thẻ bài viết</option>
                        <option
                            v-for="tag in availableMediaTags"
                            :key="tag"
                            :value="tag"
                        >
                            #{{ tag }}
                    ...
                    <div class="flex items-center gap-2">
                        <Input
                            v-model="uploadsFromDate"
                            type="date"
                            class="h-12 w-36 text-xs font-medium"
                            placeholder="Từ ngày"
                        />
                        <span class="text-xs text-foreground/50">-</span>
                        <Input
                            v-model="uploadsToDate"
                            type="date"
                            class="h-12 w-36 text-xs font-medium"
                        ...
                        >
                            <div class="flex justify-end gap-1.5">
                                <Button
                                    ...
                                    size="icon"
                                    class="size-8"
                                    title="Quản lý thẻ ảnh"
                                    @click.stop="openTagsDialog(asset)"
                                >
                            ...
                            <div class="space-y-0.5">
                                ...
                                <p
                                    ...
                                    class="line-clamp-1 text-[10px] text-white/80"
                                >
                                    {{
                                        asset.tags
                                            .map((tag) => `#${tag}`)
        ...
        <Dialog v-model:open="tagsDialogOpen">
            <DialogContent>
                <DialogHeader>
                    <DialogTitle>Quản lý thẻ ảnh</DialogTitle>
                    <DialogDescription
                        >Chọn từ danh mục Thẻ bài viết chung của workspace. Khi
                        đăng bài, thẻ bài viết cũng tự động được gắn vào
                        ảnh.</DialogDescription
                    >
                </DialogHeader>
                <div
                    class="flex max-h-72 flex-wrap gap-2 overflow-y-auto rounded-lg border p-3"
                >
                    <button
                        v-for="tag in availableMediaTags"
                        :key="tag"
                        ...
                        :class="
                            selectedAssetTags.includes(tag)
                    ...
                    <p
                        v-if="availableMediaTags.length === 0"
                        class="text-sm text-muted-foreground"
                    >
                        Chưa có Thẻ bài viết nào được cấu hình.
                    </p>
                </div>
                <DialogFooter>
                    <Button variant="outline" @click="tagsDialogOpen = false"
                        >Hủy</Button
                    >
                    <Button :disabled="tagsSaving" @click="saveTags"
                        ><IconLoader2
                            v-if="tagsSaving"
                            class="size-4 animate-spin"
                        />
