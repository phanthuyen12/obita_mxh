...
const getPublishConfig = (pp: PostPlatform): Record<string, any> | null =>
    pp.social_account_id
        ? (props.platformConfigs[pp.social_account_id]?.publishConfig ??
          props.platformConfigs[pp.platform]?.publishConfig ??
          null)
        : null;
...
const channels = computed<Channel[]>(() =>
    props.postPlatforms.map((pp) => ({
        ...
        groupIds: pp.social_account?.groups?.map((group) => group.id) ?? [],
    })),
);
</script>
