...
} from '@/types/automation/node-run-status';
...
const testHttp = useHttp<{ with_real_data: boolean }, { run_id: string }>({
    with_real_data: false,
});
const runHttp = useHttp<
    Record<string, never>,
    { run: Run; node_runs: NodeRun[] }
>({});

const fetchRun = async (runId: string): Promise<void> => {
    try {
        const data = await runHttp.get(
            showRunRoute.url({ automation: props.automationId, run: runId }),
        );
        run.value = data.run;
...
const start = async () => {
    ...
    try {
        ...
        const { run_id: runId } = await testHttp.post(
            testAutomation.url(props.automationId),
        );
...
const isZeroFetchResult = (nodeRun: NodeRun): boolean => {
    ...
    if (
        nodeRun.node_type !== NodeType.FetchRss &&
        nodeRun.node_type !== NodeType.HttpRequest
    )
        return false;
...
<template>
    <div class="flex flex-col gap-5 px-5 pt-5 pb-12">
        <div class="min-w-0">
            <span
                v-if="run?.is_dry_run"
                class="inline-flex -rotate-3 items-center rounded-md border-2 border-foreground bg-amber-200 px-2 py-0.5 text-[10px] font-black tracking-widest text-foreground uppercase shadow-[2px_2px_0_var(--foreground)]"
            >
                {{ $t('automations.test.dry_badge') }}
            </span>
            <p
                class="text-sm text-foreground/60"
                :class="{ 'mt-2': run?.is_dry_run }"
            >
                {{
                    run?.is_dry_run === false
                        ? $t('automations.test.real_data_hint')
                        : $t('automations.test.description')
                }}
            </p>
        </div>

        <div
            class="flex items-center justify-between gap-3 rounded-xl border-2 border-foreground bg-card p-3 shadow-[3px_3px_0_var(--foreground)]"
        >
            ...
            <Button
                size="sm"
                :disabled="isStarting || !!configIssue"
                @click="runTest"
            >
        ...
        <div
            v-if="run === null && isStarting"
            class="flex items-center gap-2.5 text-sm font-medium text-foreground/70"
        >
        ...
        <ul v-if="nodeRuns.length > 0" class="space-y-3">
            ...
            >
                <div class="flex items-center gap-3 p-4">
                    <div
                        :class="[
                            'inline-flex size-10 shrink-0 -rotate-3 items-center justify-center rounded-xl border-2 border-foreground shadow-2xs',
                            nodeStatusTile(nodeRun.status),
                        ]"
                    >
                        <component
                            :is="nodeStatusIcon(nodeRun.status)"
                            :class="[
                                'size-5',
                    ...
                    <div class="min-w-0 flex-1">
                        <p class="text-base leading-tight font-bold capitalize">
                            {{
                                $t(`automations.node_type.${nodeRun.node_type}`)
                            }}
                        </p>
                        <p
                            class="text-xs font-semibold tracking-wider text-foreground/50 uppercase"
                        >
                ...
                <div
                    v-if="nodeRun.error || nodeRun.output"
                    class="border-t-2 border-foreground/10 px-4 pt-3 pb-4"
                >
                    <p
                        v-if="nodeRun.error"
                        class="rounded-lg border-2 border-rose-700 bg-rose-50 p-3 text-sm font-medium text-rose-800"
                    >
                        <span
                            class="text-xs font-black tracking-wider uppercase"
                            >{{ $t('automations.test.node_error') }}:</span
                        >
                    ...
                    <details
                        v-if="nodeRun.output && !isZeroFetchResult(nodeRun)"
                        class="group"
                        :class="{ 'mt-3': nodeRun.error }"
                    >
                        <summary
                            class="flex cursor-pointer items-center gap-1.5 text-xs font-black tracking-wider text-foreground/60 uppercase hover:text-foreground"
                        >
                            <IconChevronRight
