...
} from '@/types/automation/trigger-type';
...
>({
    ...
    trigger_type:
        (props.data.trigger_type as TriggerTypeValue) ?? TriggerType.Schedule,
    cron: (props.data.cron as string) ?? '0 9 * * *',
    schedule_minute: snapToFiveMinutes(
        Number(
            props.data.schedule_minute ??
                normalizeScheduleData(props.data as ScheduleData)
                    .schedule_minute,
        ) || 0,
    ),
    schedule_timezone:
        (props.data.schedule_timezone as string) ?? date.getUserTimezone(),
});

const num = (
    key: keyof typeof local.value,
    fallback: number,
    min: number,
    max: number,
) => clamp(Number(local.value[key]) || fallback, min, max);

const proxy = (key: 'schedule_hour' | 'schedule_minute', max: number) =>
    computed({
        get: () => pad2(num(key, 0, 0, max)),
        set: (v: string) => {
            local.value[key] = Number(v) || 0;
        },
    });
...
const scheduleSummaryLine = computed(() => {
    ...
    const pinsClockTime =
        local.value.schedule_field === ScheduleField.Days ||
    ...
    <div class="space-y-3">
        <div>
            <Label class="mb-1 block">{{
                $t('automations.config.trigger.type')
            }}</Label>
            <Select v-model="local.trigger_type">
                ...
                <SelectContent>
                    <SelectItem :value="TriggerType.Schedule">{{
                        $t('automations.config.trigger.types.schedule')
                    }}</SelectItem>
                    <SelectItem :value="TriggerType.PostPublished">{{
                        $t('automations.config.trigger.types.post_published')
                    }}</SelectItem>
                    <SelectItem :value="TriggerType.PostScheduled">{{
                        $t('automations.config.trigger.types.post_scheduled')
                    }}</SelectItem>
                </SelectContent>
            </Select>
        ...
        <template v-if="local.trigger_type === TriggerType.Schedule">
            <div>
                <Label class="mb-1 block">{{
                    $t('automations.config.trigger.schedule.field')
                }}</Label>
                <Select v-model="local.schedule_field">
                    ...
                    <SelectContent>
                        <SelectItem :value="ScheduleField.Minutes">{{
                            $t(
                                'automations.config.trigger.schedule.fields.minutes',
                            )
                        }}</SelectItem>
                        <SelectItem :value="ScheduleField.Hours">{{
                            $t(
                                'automations.config.trigger.schedule.fields.hours',
                            )
                        }}</SelectItem>
                        <SelectItem :value="ScheduleField.Days">{{
                            $t(
                                'automations.config.trigger.schedule.fields.days',
                            )
                        }}</SelectItem>
                        <SelectItem :value="ScheduleField.Weeks">{{
                            $t(
                                'automations.config.trigger.schedule.fields.weeks',
                            )
                        }}</SelectItem>
                        <SelectItem :value="ScheduleField.Months">{{
                            $t(
                                'automations.config.trigger.schedule.fields.months',
                            )
                        }}</SelectItem>
                    </SelectContent>
                </Select>
            ...
            <div v-if="local.schedule_field === ScheduleField.Minutes">
                <Label class="mb-1 block">{{
                    $t('automations.config.trigger.schedule.minutes_interval')
                }}</Label>
                <Input
                    type="number"
                    v-model.number="local.schedule_minutes_interval"
                    min="1"
                    max="59"
                />
            </div>
            ...
            <template v-if="local.schedule_field === ScheduleField.Hours">
                <div>
                    <Label class="mb-1 block">{{
                        $t('automations.config.trigger.schedule.hours_interval')
                    }}</Label>
                    <Input
                        type="number"
                        v-model.number="local.schedule_hours_interval"
                        min="1"
                        max="23"
                    />
                </div>
                <div>
                    <Label class="mb-1 block">{{
                        $t('automations.config.trigger.schedule.minute')
                    }}</Label>
            ...
            <template v-if="local.schedule_field === ScheduleField.Days">
                <div>
                    <Label class="mb-1 block">{{
                        $t('automations.config.trigger.schedule.days_interval')
                    }}</Label>
                    <Input
                        type="number"
                        v-model.number="local.schedule_days_interval"
                        min="1"
                        max="31"
                    />
                </div>
                <div class="grid grid-cols-2 gap-3">
                    <div>
                        <Label class="mb-1 block">{{
                            $t('automations.config.trigger.schedule.hour')
                        }}</Label>
            ...
            <template v-if="local.schedule_field === ScheduleField.Weeks">
                <div>
                    <Label class="mb-1 block">{{
                        $t('automations.config.trigger.schedule.weekdays')
                    }}</Label>
                    <div class="flex flex-wrap gap-1.5">
                        ...
                        >
                            {{
                                $t(
                                    `automations.config.trigger.schedule.weekday_names.${day.label}`,
                                )
                            }}
                        </button>
                    </div>
            ...
            <template v-if="local.schedule_field === ScheduleField.Months">
                <div>
                    <Label class="mb-1 block">{{
                        $t('automations.config.trigger.schedule.day_of_month')
                    }}</Label>
                    <Input
                        type="number"
                        v-model.number="local.schedule_day_of_month"
                        min="1"
                        max="31"
                    />
                </div>
