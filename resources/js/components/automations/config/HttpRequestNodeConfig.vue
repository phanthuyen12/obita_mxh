...
const headerRows = ref<HeaderRow[]>(
    Object.entries((props.data.headers as Record<string, string>) ?? {}).map(
        ([name, value]) => ({
            name,
            value: String(value),
        }),
...
const supportsBody = computed(() =>
    HTTP_METHODS_WITH_BODY.includes(local.value.method),
);
...
<template>
    <div class="space-y-4">
        <div class="grid grid-cols-[110px_1fr] gap-2">
            <div>
                <Label class="mb-1 block">{{
                    $t('automations.config.http_request.method')
                }}</Label>
            ...
            <div>
                <Label class="mb-1 block">{{
                    $t('automations.config.http_request.url')
                }}</Label>
                <Input
                    v-model="local.url"
        ...
        <div>
            <Label class="mb-1 block">{{
                $t('automations.config.http_request.auth_type')
            }}</Label>
            <Select v-model="local.auth_type">
                ...
                <SelectContent>
                    <SelectItem :value="AuthType.None">{{
                        $t('automations.config.http_request.auth.none')
                    }}</SelectItem>
                    <SelectItem :value="AuthType.Bearer">{{
                        $t('automations.config.http_request.auth.bearer')
                    }}</SelectItem>
                    <SelectItem :value="AuthType.Basic">{{
                        $t('automations.config.http_request.auth.basic')
                    }}</SelectItem>
                    <SelectItem :value="AuthType.ApiKey">{{
                        $t('automations.config.http_request.auth.api_key')
                    }}</SelectItem>
                </SelectContent>
            </Select>
        </div>
        ...
        <div v-if="local.auth_type === AuthType.Bearer">
            <Label class="mb-1 block">{{
                $t('automations.config.http_request.bearer_token')
            }}</Label>
            <Input
                v-model="local.auth_token"
                type="password"
                autocomplete="off"
                placeholder="sk-…"
            />
        ...
        <template v-if="local.auth_type === AuthType.Basic">
            <div>
                <Label class="mb-1 block">{{
                    $t('automations.config.http_request.basic_username')
                }}</Label>
            ...
            <div>
                <Label class="mb-1 block">{{
                    $t('automations.config.http_request.basic_password')
                }}</Label>
                <Input
                    v-model="local.auth_password"
                    type="password"
                    autocomplete="off"
                />
        ...
        <template v-if="local.auth_type === AuthType.ApiKey">
            <div>
                <Label class="mb-1 block">{{
                    $t('automations.config.http_request.api_key_header')
                }}</Label>
                <Input
                    v-model="local.auth_header_name"
                    placeholder="X-API-Key"
                />
            </div>
            <div>
                <Label class="mb-1 block">{{
                    $t('automations.config.http_request.api_key_value')
                }}</Label>
                <Input
                    v-model="local.auth_token"
                    type="password"
        ...
        <div v-if="supportsBody" v-show="!editorExpanded">
            <Label class="mb-1 block">{{
                $t('automations.config.http_request.body_template')
            }}</Label>
            <div class="h-36">
                <CodeEditor
        ...
        <div>
            <Label class="mb-1 block">{{
                $t('automations.config.http_request.headers')
            }}</Label>
            <div class="space-y-2">
                <div
                ...
                >
                    <Input
                        ...
                        class="flex-1"
                        :placeholder="
                            $t('automations.config.http_request.header_name')
                        "
                    />
                    <Input
                        v-model="row.value"
                        class="flex-1"
                        :placeholder="
                            $t('automations.config.http_request.header_value')
                        "
                    />
                    <Button
                        type="button"
        ...
        <div class="border-t-2 border-foreground/10 pt-4">
            ...
            <div class="space-y-3">
                <div>
                    <Label class="mb-1 block">{{
                        $t('automations.config.http_request.items_path')
                    }}</Label>
                    <Input
                        v-model="local.items_path"
                        placeholder="data.items"
                    />
                    <p class="mt-1 text-xs text-foreground/50">
                        {{
                            $t(
                                'automations.config.http_request.items_path_hint',
                            )
                        }}
                    </p>
                ...
                <div>
                    <Label class="mb-1 block">{{
                        $t('automations.config.http_request.item_date_path')
                    }}</Label>
                    <Input
                        v-model="local.item_date_path"
                        placeholder="published_at"
                    />
                    <p class="mt-1 text-xs text-foreground/50">
                        {{
                            $t(
                                'automations.config.http_request.item_date_path_hint',
                            )
                        }}
                    </p>
                    <InputError
                        :message="errors?.item_date_path"
                        class="mt-1"
                    />
                </div>
                <div>
                    <Label class="mb-1 block">{{
                        $t('automations.config.http_request.item_key_path')
                    }}</Label>
                    <Input v-model="local.item_key_path" placeholder="id" />
                    <p class="mt-1 text-xs text-foreground/50">
                        {{
                            $t(
                                'automations.config.http_request.item_key_path_hint',
                            )
                        }}
                    </p>
