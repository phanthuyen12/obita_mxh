...
const local = ref<FetchRssConfig>({
    ...
    discovered_fields:
        (props.data.discovered_fields as DiscoveredField[]) ?? [],
});
...
const inspect = async () => {
    ...
    try {
        ...
        const { fields } = await inspectHttp.post(
            inspectFeed.url(automationId),
        );
        local.value.discovered_fields = fields;
    } catch {
        inspectError.value = trans(
            'automations.config.fetch_rss.inspect_error',
        );
    } finally {
    ...
    <div class="space-y-4">
        <div>
            <Label class="mb-1 block">{{
                $t('automations.config.fetch_rss.feed_url')
            }}</Label>
            <Input
                v-model="local.feed_url"
                placeholder="https://example.com/feed.xml"
            />
            <InputError :message="errors?.feed_url" class="mt-1" />
            <p class="mt-1 text-xs text-foreground/50">
                {{ $t('automations.config.fetch_rss.feed_url_hint') }}
            </p>
        </div>
        ...
        <div>
            <Button
                ...
                variant="outline"
                size="sm"
                :disabled="isInspecting || local.feed_url.trim() === ''"
                @click="inspect"
            >
                ...
                {{
                    isInspecting
                        ? $t('automations.config.fetch_rss.inspecting')
                        : $t('automations.config.fetch_rss.inspect')
                }}
            </Button>
            <p class="mt-1 text-xs text-foreground/50">
                {{ $t('automations.config.fetch_rss.inspect_hint') }}
            </p>
            <InputError
                v-if="inspectError"
        ...
        <div v-if="local.discovered_fields.length > 0">
            <Label class="mb-1 block">{{
                $t('automations.config.fetch_rss.discovered_fields')
            }}</Label>
            <ul
                class="max-h-48 space-y-1.5 overflow-y-auto rounded-md border border-border p-2"
            >
                <li
                    v-for="field in local.discovered_fields"
                    :key="field.path"
