...
const summary = computed(() => {
    ...
    if (
        props.data.mode === PublishMode.Scheduled &&
        props.data.scheduled_offset != null
    ) {
        return trans('automations.config.publish.offset_summary', {
            mode,
            offset: String(props.data.scheduled_offset),
        });
    }
    return mode;
});
...
<template>
    ...
    >
        <div class="automation-node__header">
            <div
                class="automation-node__icon-tile automation-node__icon-tile--emerald"
            >
                <IconSend :size="16" />
            </div>
            <span class="automation-node__title">{{
                $t('automations.nodes.publish')
            }}</span>
        </div>
