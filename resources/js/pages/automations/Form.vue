...
const hasTriggerNode = computed(() =>
    nodes.value.some((node) => node.type === NodeType.Trigger),
);
...
const createNodeAt = (type: string, position: { x: number; y: number }) => {
    if (type === NodeType.Trigger && hasTriggerNode.value) {
        toast.error(trans('automations.errors.must_have_one_trigger'));
        return;
    }
...
const nextNodePosition = (): XYPosition => {
...
const addNodeFromPanel = (type: string) => {
    createNodeAt(type, nextNodePosition());
};
...
<template>
    ...
    <AppLayout full-width>
        <div class="flex min-h-0 flex-1 flex-col bg-background">
            ...
            >
                <div
                    class="bg-gold inline-flex size-14 items-center justify-center rounded-2xl border-2 border-foreground shadow-2xs"
                >
                    <IconDeviceDesktop
...
/* Canvas surface — n8n uses a near-white gray (#f5f5f5) as the canvas background.
   King Social Hub uses off-white for brand consistency. Dots use n8n's
...
/* Controls — keep King Social Hub's ink-border, hard-shadow identity but adopt n8n's
...
/* Edges — King Social Hub ink-on-cream. Default uses the foreground color at 55% so
...
/* Handles — solid ink-bordered dots in the King Social Hub brutalist style. Offset by
