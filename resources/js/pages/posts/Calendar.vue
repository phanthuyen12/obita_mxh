...
const getStatusColor = (post: Post): string => {
    if (post.status === PostStatus.Failed) {
        return 'border-rose-500 bg-rose-100 text-rose-950 dark:bg-rose-950/50 dark:text-rose-100';
    }
    if (
        post.status === PostStatus.Published ||
        post.status === PostStatus.PartiallyPublished
    ) {
        return 'border-emerald-500 bg-emerald-100 text-emerald-950 dark:bg-emerald-950/50 dark:text-emerald-100';
    }
    if (post.status === PostStatus.Publishing) {
        return 'border-fuchsia-500 bg-fuchsia-100 text-fuchsia-950 dark:bg-fuchsia-950/50 dark:text-fuchsia-100';
    }
    if (post.status === PostStatus.Scheduled) {
        return 'border-violet-500 bg-violet-100 text-violet-950 dark:bg-violet-950/50 dark:text-violet-100';
    }
    if (post.workflow_status === 'pending_review') {
        return 'border-orange-500 bg-orange-100 text-orange-950 dark:bg-orange-950/50 dark:text-orange-100';
    }
    if (post.workflow_status === 'approved') {
        return 'border-sky-500 bg-sky-100 text-sky-950 dark:bg-sky-950/50 dark:text-sky-100';
    }
    if (post.workflow_status === 'rejected') {
        return 'border-amber-500 bg-amber-100 text-amber-950 dark:bg-amber-950/50 dark:text-amber-100';
    }

    const colors: Record<string, string> = {
        draft: 'bg-card text-foreground',
    };
    return colors[post.status] ?? colors.draft;
};
...
<template>
    ...
    <AppLayout :fullWidth="true">
        <div class="flex h-full flex-col">
            ...
            <section
                class="grid shrink-0 grid-cols-2 gap-3 border-b-2 border-foreground/10 bg-background p-4 sm:grid-cols-3 xl:grid-cols-5"
            >
                <article
                    class="flex items-center gap-3 rounded-xl border bg-card p-4 shadow-2xs"
                >
                    <span
                        class="rounded-lg bg-blue-100 p-2 text-blue-600 dark:bg-blue-950"
                    >
                ...
                >
                    <span
                        class="rounded-lg bg-orange-100 p-2 text-orange-600 dark:bg-orange-950"
                    >
                        <IconClock class="size-5" />
                    </span>
                    <div>
                        <p class="text-xs text-muted-foreground">Đợi duyệt</p>
                        <p class="text-2xl font-bold tabular-nums">
                ...
                >
                    <span
                        class="rounded-lg bg-violet-100 p-2 text-violet-600 dark:bg-violet-950"
                    >
                        <IconCalendarTime class="size-5" />
                    </span>
                    <div>
                        <p class="text-xs text-muted-foreground">Đợi đăng</p>
                        <p class="text-2xl font-bold tabular-nums">
                            {{ stats.scheduled }}
                        </p>
                    </div>
                ...
                >
                    <span
                        class="rounded-lg bg-emerald-100 p-2 text-emerald-600 dark:bg-emerald-950"
                    >
                ...
                <article
                    class="col-span-2 flex items-center gap-3 rounded-xl border bg-card p-4 shadow-2xs sm:col-span-1"
                >
                    <span
                        class="rounded-lg bg-rose-100 p-2 text-rose-600 dark:bg-rose-950"
                    >
                        <IconAlertTriangle class="size-5" />
                    </span>
                    <div>
                        <p class="text-xs text-muted-foreground">Đã lỗi</p>
                        <p class="text-2xl font-bold tabular-nums">
                            {{ stats.failed }}
                        </p>
                    </div>
            ...
            <aside
                class="flex shrink-0 flex-wrap items-center gap-x-5 gap-y-2 border-b-2 border-foreground/10 bg-card px-4 py-3 text-xs"
                aria-label="Chú thích trạng thái bài viết"
            >
                <strong class="text-foreground">Chú thích:</strong>
                <span class="inline-flex items-center gap-1.5">
                    <i class="size-2.5 rounded-full bg-orange-500"></i>Chờ duyệt
                </span>
                <span class="inline-flex items-center gap-1.5">
                    <i class="size-2.5 rounded-full bg-sky-500"></i>Đã duyệt
                </span>
                <span class="inline-flex items-center gap-1.5">
                    <i class="size-2.5 rounded-full bg-amber-500"></i>Đang chỉnh
                    sửa
                </span>
                <span class="inline-flex items-center gap-1.5">
                    <i class="size-2.5 rounded-full bg-violet-500"></i>Đợi đăng
                </span>
                <span class="inline-flex items-center gap-1.5">
                    <i class="size-2.5 rounded-full bg-fuchsia-500"></i>Đang
                    đăng
                </span>
                <span class="inline-flex items-center gap-1.5">
                    <i class="size-2.5 rounded-full bg-emerald-500"></i>Đã đăng
                </span>
                <span class="inline-flex items-center gap-1.5">
                    <i class="size-2.5 rounded-full bg-rose-500"></i>Đã lỗi
                </span>
            </aside>
                            ...
                            >
                                ...
                                >
                                    <div class="min-w-0 flex-1">
                                        ...
                                        <span
                                            v-if="post.folder"
                                            class="mt-2 inline-flex rounded-full border bg-card px-2 py-0.5 text-[10px] font-medium"
