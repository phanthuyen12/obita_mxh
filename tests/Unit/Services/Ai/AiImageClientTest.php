...
test('generate uses Dify workflow when it is the configured image provider', function (): void {
    config()->set([
        'ai.default_for_images' => 'dify',
    ...
    Http::fake([
        'https://dify.example/v1/workflows/run' => Http::response([
            'data' => [
                'status' => 'succeeded',
                'outputs' => [
                    'image_base64' => base64_encode('generated-image-bytes'),
                ],
            ],
        ]),
    ]);

    $result = (new AiImageClient)->generate(['coffee campaign'], ImageStyle::Illustration);
    ...
    Http::assertSent(fn ($request): bool => $request['inputs']['task'] === 'image_generation'
        && $request['inputs']['orientation'] === 'portrait'
        && filled($request['inputs']['prompt']));
});
