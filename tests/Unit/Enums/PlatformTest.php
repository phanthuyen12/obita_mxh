...
test('Omnichat-only platforms are excluded from publishing', function () {
    expect(Platform::Shopee->supportsPublishing())->toBeFalse()
        ->and(Platform::Lazada->supportsPublishing())->toBeFalse()
        ->and(Platform::ZaloOa->supportsPublishing())->toBeFalse()
        ->and(Platform::LinkedIn->supportsPublishing())->toBeTrue()
        ->and(Platform::publishingValues())->not->toContain(
            Platform::Shopee->value,
            Platform::Lazada->value,
            Platform::ZaloOa->value,
        );
});
