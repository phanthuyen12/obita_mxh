...
beforeEach(function () {
    config()->set('services.facebook.login_config_id', '2284279519065535');
...
test('facebook connect redirects to oauth provider', function () {
    ...
    $oauthUrl = $response->headers->get('Location');
    parse_str((string) parse_url($oauthUrl, PHP_URL_QUERY), $oauthQuery);

    expect($oauthQuery)
        ->not->toHaveKey('scope')
        ->toMatchArray([
            'test' => '1',
            'config_id' => '2284279519065535',
...
test('facebook oauth callback creates account with single page', function () {
    ...
    Queue::assertPushed(SubscribeFacebookPageToWebhooks::class, fn (SubscribeFacebookPageToWebhooks $job): bool => SocialAccount::query()
        ->whereKey($job->socialAccountId)
        ->where('platform_user_id', 'page_123')
        ->exists());
});
