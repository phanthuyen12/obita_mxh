...
use App\Actions\Post\SyncPostMediaTags;
...
test('asset tags can be updated and used to filter the media library', function () {
    foreach (['Khuyến mãi', 'Facebook'] as $tag) {
        Tag::query()->create([
    ...
    $matching = $this->workspace->addMedia(UploadedFile::fake()->image('campaign.jpg'), 'assets');
    $this->workspace->addMedia(UploadedFile::fake()->image('other.jpg'), 'assets');

    $this->actingAs($this->user)
        ->putJson(route('app.assets.tags.update', $matching), [
            'tags' => ['Khuyến mãi', 'Facebook'],
        ])
        ->assertOk()
        ->assertJsonPath('data.tags.0', 'Khuyến mãi')
        ->assertJsonPath('data.tags.1', 'Facebook');

    $this->actingAs($this->user)
        ->getJson(route('app.assets.search', ['tag' => 'Khuyến mãi']))
        ->assertOk()
        ->assertJsonCount(1, 'data')
        ->assertJsonPath('data.0.id', $matching->id)
        ->assertJsonPath('tag_options.1', 'Khuyến mãi');
});

test('post topic tags are merged into selected media tags', function () {
    $media = $this->workspace->addMedia(UploadedFile::fake()->image('campaign.jpg'), 'assets');
    $existingTag = Tag::factory()->create([
        'workspace_id' => $this->workspace->id,
        'name' => 'Sản phẩm',
        'slug' => 'san-pham',
    ]);
    $media->tags()->attach($existingTag);
    ...
    expect($media->fresh()->tags()->pluck('name')->all())
        ->toEqualCanonicalizing(['Sản phẩm', 'Khuyến mãi', 'CEO']);
});
...
test('assets search hides media in folders without view permission', function () {
    $nonOwnerMember = User::factory()->create(['account_id' => $this->account->id]);
    $this->workspace->members()->attach($nonOwnerMember->id, [
        'role' => Role::Member->value,
        'can_content' => true,
    ]);
    $workflow = ContentWorkflow::factory()->create(['workspace_id' => $this->workspace->id]);
    $workflow->members()->attach($nonOwnerMember->id, ['can_write' => true]);
    $nonOwnerMember->update(['current_workspace_id' => $this->workspace->id]);

    $folder = Folder::factory()->create([
        'workspace_id' => $this->workspace->id,
        'created_by' => $this->user->id,
    ]);
    $hidden = $this->workspace->addMedia(UploadedFile::fake()->image('hidden.jpg'), 'assets');
    $hidden->update(['folder_id' => $folder->id]);
    $visible = $this->workspace->addMedia(UploadedFile::fake()->image('visible.jpg'), 'assets');

    $response = $this->actingAs($nonOwnerMember)->getJson(route('app.assets.search'));

    $response->assertOk()->assertJsonCount(1, 'data');
    $response->assertJsonPath('data.0.id', $visible->id);
});

test('assets search returns folder media when user has inherited view permission', function () {
    $folder = Folder::factory()->create([
        'workspace_id' => $this->workspace->id,
        'created_by' => $this->user->id,
    ]);
    ...
    $media = $this->workspace->addMedia(UploadedFile::fake()->image('allowed.jpg'), 'assets');
    $media->update(['folder_id' => $folder->id]);

    $response = $this->actingAs($this->user)
        ->getJson(route('app.assets.search', ['folder_id' => $folder->id]));
