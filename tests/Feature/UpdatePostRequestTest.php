...
test('a post cannot select more than one wordpress website', function () {
    $wordpressPlatforms = collect(['https://first.test', 'https://second.test'])
        ->map(function (string $url): PostPlatform {
            $account = SocialAccount::factory()->create([
                'workspace_id' => $this->workspace->id,
                'platform' => Platform::WordPress,
                'platform_user_id' => $url,
            ]);
            ...
            return PostPlatform::factory()->create([
                ...
                'platform' => Platform::WordPress,
                'content_type' => ContentType::WordPressArticle,
            ]);
        });
    ...
    $this->actingAs($this->user)
        ->put(route('app.posts.update', $this->post), [
            ...
            'platforms' => $wordpressPlatforms
                ->map(fn (PostPlatform $platform): array => ['id' => $platform->id])
                ->all(),
        ])
        ->assertSessionHasErrors([
            'platforms' => 'Mỗi bài viết chỉ được chọn một website WordPress.',
        ]);
});
