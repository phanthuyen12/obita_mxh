...
test('execute keeps an active approval workflow and resets its review state', function () {
    $user = User::factory()->create();
    $workspace = Workspace::factory()->create(['user_id' => $user->id]);
    ...
    $original = Post::factory()->create([
        ...
        'workflow_note' => 'Approved original',
    ]);

    $copy = DuplicatePost::execute($original, $user);

    expect($copy->content_workflow_id)->toBe($workflow->id)
        ->and($copy->workflow_status)->toBe('draft')
        ->and($copy->workflow_note)->toBeNull();
});
