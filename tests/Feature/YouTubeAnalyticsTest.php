...
test('post metrics fetcher supports youtube platforms', function () {
    ...
    $postPlatform = PostPlatform::factory()->youtube()->published()->create([
        'post_id' => $post->id,
    ...
    expect($metrics)->not->toHaveKey('unsupported')
        ->and($metrics)->toContain(['label' => 'Views', 'value' => 1000])
        ->and($metrics)->toContain(['label' => 'Likes', 'value' => 88])
        ->and($metrics)->toContain(['label' => 'Comments', 'value' => 12])
        ->and($metrics)->toContain(['label' => 'Shares', 'value' => 4]);
});
