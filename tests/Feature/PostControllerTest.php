...
test('posts index exposes both configured topics and tags for topic tag filter', function () {
    Tag::query()->create([
        'workspace_id' => $this->workspace->id,
        'name' => 'Khuyến mãi',
        'slug' => 'khuyen-mai',
    ...
    $this->actingAs($this->user)
        ...
        ->assertInertia(fn ($page) => $page
            ->has('topicTags', 2)
            ->where('topicTags.0.name', 'IT')
            ->where('topicTags.1.name', 'Khuyến mãi')
        );
});
...
test('posts index filters by status folder platform workflow and review state', function () {
    $workflow = ContentWorkflow::factory()->create(['workspace_id' => $this->workspace->id]);
    ...
    $matchingPost->tags()->attach(Tag::query()->create([
        'workspace_id' => $this->workspace->id,
        'name' => 'CEO',
        'slug' => 'ceo',
        'color' => '#f59e0b',
    ...
    $this->actingAs($this->user)
        ...
        ->assertInertia(fn ($page) => $page
            ...
            ->where('filters.folder_id', $folder->id)
            ->where('filters.platform', Platform::LinkedIn->value)
            ->where('filters.topic_tag', 'CEO')
        );
});
...
test('create excludes Omnichat-only social accounts', function () {
...
test('store post treats Omnichat-only accounts as unavailable for publishing', function () {
    $this->socialAccount->delete();
    SocialAccount::factory()->create([
        'workspace_id' => $this->workspace->id,
        'platform' => Platform::Shopee,
...
test('edit post shows edit page', function () {
    ...
    $group->socialAccounts()->attach($this->socialAccount);
    ...
    $response->assertInertia(fn ($page) => $page
        ...
        ->has('post')
        ->has('platformConfigs.linkedin')
        ->has('socialAccountGroups', 1)
        ->where('socialAccountGroups.0.name', 'Nhóm bán hàng')
        ->where('post.post_platforms.0.social_account.groups.0.id', $group->id)
    );
});
...
test('edit exposes every connected wordpress website', function () {
    $sites = collect([
        ['name' => 'MBM Food', 'url' => 'https://mbmfood.test'],
        ['name' => 'MyVo', 'url' => 'https://myvo.test'],
    ])->map(fn (array $website): WordPressSite => WordPressSite::query()->create([
        'workspace_id' => $this->workspace->id,
        'name' => $website['name'],
        'url' => $website['url'],
        'username' => 'editor',
    ...
    $this->actingAs($this->user)
        ...
        ->assertInertia(fn ($page) => $page
            ->component('posts/Edit')
            ->has('wordPressSites', 2)
            ->where('wordPressSites.0.name', 'MBM Food')
            ->where('wordPressSites.1.name', 'MyVo')
            ->where('post.post_platforms', function ($platforms) use ($sites): bool {
                $siteIds = collect($platforms)
                    ->where('platform', 'wordpress')
                    ->pluck('social_account.meta.site_id')
                    ->sort()
                    ->values();

                return $siteIds->all() === $sites->pluck('id')->sort()->values()->all();
            })
        );
});

test('edit exposes workflow permissions for the current workflow member', function () {
    $workflowMember = User::factory()->create([
        'account_id' => $this->workspace->account_id,
        'current_workspace_id' => $this->workspace->id,
    ]);
    $this->workspace->members()->attach($workflowMember->id, ['role' => Role::Member->value]);
    ...
    $workflow->members()->attach($workflowMember->id, [
        'can_write' => true,
        'can_review' => false,
        'can_publish' => true,
    ]);
    ...
    $post = Post::factory()->create([
        ...
        'content_workflow_id' => $workflow->id,
        'workflow_status' => 'pending_review',
        'workflow_note' => 'Please check wording.',
    ]);

    $this->actingAs($workflowMember)
        ->get(route('app.posts.edit', $post))
        ->assertOk()
        ->assertInertia(fn ($page) => $page
            ->component('posts/Edit')
            ->where('workflow.name', 'Editorial approval')
            ->where('workflow.status', 'pending_review')
            ->where('workflow.note', 'Please check wording.')
            ->where('workflow.can_write', true)
            ->where('workflow.can_review', false)
            ->where('workflow.can_publish', true)
        );
});

test('a workflow writer submits a proposed publishing time for review', function () {
    $writer = User::factory()->create([
        'account_id' => $this->workspace->account_id,
        'current_workspace_id' => $this->workspace->id,
    ]);
    $this->workspace->members()->attach($writer->id, [
    ...
    $workflow->members()->attach($writer->id, [
        'can_write' => true,
        'can_review' => false,
        'can_publish' => false,
    ]);
    ...
    $this->actingAs($writer)
        ->post(route('app.posts.workflow.submit', $post), [
            'scheduled_at' => $scheduledAt->toIso8601String(),
        ])
        ->assertRedirect();

    expect($post->fresh()->workflow_status)->toBe('pending_review')
        ->and($post->fresh()->status)->toBe(PostStatus::Draft)
        ->and($post->fresh()->scheduled_at?->toDateTimeString())->toBe($scheduledAt->utc()->toDateTimeString());
});

test('a workflow writer must choose a publishing time before submitting for review', function () {
    $writer = User::factory()->create([
        'account_id' => $this->workspace->account_id,
        'current_workspace_id' => $this->workspace->id,
    ]);
    ...
    $workflow->members()->attach($writer->id, ['can_write' => true]);
    ...
    expect($post->fresh()->workflow_status)->toBe('draft')
        ->and($post->fresh()->scheduled_at)->toBeNull();
});

test('a workflow writer cannot set a publishing time or schedule a post', function () {
    $writer = User::factory()->create([
        'account_id' => $this->workspace->account_id,
        'current_workspace_id' => $this->workspace->id,
    ]);
...
test('a draft can select an active approval workflow', function () {
    $workflow = ContentWorkflow::factory()->create([
        'workspace_id' => $this->workspace->id,
        'social_account_id' => $this->socialAccount->id,
        'created_by' => $this->user->id,
    ...
    $this->actingAs($this->user)
        ->get(route('app.posts.edit', $post))
        ->assertInertia(fn ($page) => $page
            ->where('contentWorkflows.0.id', $workflow->id)
            ->where('contentWorkflows.0.name', $workflow->name)
        );
    ...
    expect($post->fresh()->content_workflow_id)->toBe($workflow->id)
        ->and($post->fresh()->workflow_status)->toBe('draft');
});

test('approving a workflow post keeps it as a draft ready to publish', function () {
    $reviewer = User::factory()->create([
        'account_id' => $this->workspace->account_id,
        'current_workspace_id' => $this->workspace->id,
    ]);
    $this->workspace->members()->attach($reviewer->id, ['role' => Role::Member->value]);
    ...
    $workflow->members()->attach($reviewer->id, [
        'can_write' => false,
        'can_review' => true,
        'can_publish' => true,
    ]);
    ...
    $post = Post::factory()->create([
        ...
        'content_workflow_id' => $workflow->id,
        'workflow_status' => 'pending_review',
        'workflow_note' => 'Ready for final approval.',
        'scheduled_at' => $scheduledAt,
    ]);
    ...
    expect($post->workflow_status)->toBe('approved')
        ->and($post->workflow_note)->toBeNull()
        ->and($post->status)->toBe(PostStatus::Scheduled)
        ->and($post->scheduled_at?->toDateTimeString())->toBe($scheduledAt->utc()->toDateTimeString());
});
...
test('member can create post', function () {
    ...
    $workflow = ContentWorkflow::factory()->create([
        'workspace_id' => $this->workspace->id,
        'name' => 'Luồng duyệt bài',
        'is_active' => true,
    ]);
    $workflow->members()->attach($member->id, [
        'can_write' => true,
        'can_review' => false,
        'can_publish' => false,
    ]);
